...
3. **Granularity follows disclosure (the "trace" method).** The analyst's
   ...
   closer (the ArcSoft benchmark). Every trace carries a precise anchor
   (filing · page), and traces must be *chained*: single clues become
   predictive only when cross-linked (data clue × news clue × narrative
   clue → one verdict → one parameter, with the chain visible in the
   report so every number can answer "on what grounds"). The *criterion*
   for a split is growth-logic heterogeneity: two branches belong apart
   whenever their demand drivers or news sensitivity differ (US
   enterprise-AI demand vs European AI-sovereignty demand are different
   stories even inside one product line). Backcast numbers are
   imprecise but still used — with conservative parameters, wider
   intervals, and low-confidence labels (same philosophy as data-grade
   C). Macro is the same discipline one level up: a news item matters
   only as a *transmission chain to a specific branch* (export controls
   → domestic labs lease overseas capacity → that lessor's
   overseas-segment revenue revises up), and when the chain from news to
   branch is unclear, the pipeline brainstorms with the user instead of
   guessing. The deliverable is the full ArcSoft arc: deep per-segment
   models (each with its own traces and chains) → parallel segments →
   segment-to-total consolidated report with per-branch confidence
   grading.
