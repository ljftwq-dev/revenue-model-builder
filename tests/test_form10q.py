"""Tests for revenue_model.form10q — 10-Q filings from EDGAR into the
queue. All offline (injected getters + fake pdf_writer); the one
end-to-end test needs the [pdf] extra and skips without it."""
from datetime import datetime
from pathlib import Path

import pytest

from revenue_model.form10q import (digest_10q, fetch_10q_filings,
                                   quarter_tag, queue_10q)
...
class TestEndToEnd:
    def test_real_writer_then_digest(self, tmp_path):
        ...
        r = digest_10q(recs[:1], backend, queue_dir=tmp_path)
