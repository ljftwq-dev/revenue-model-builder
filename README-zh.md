...
- **v0.20+**：更多外部锚（ChartMogul SaaS 阶段表）；每年 1 月随 Damodaran
  更新刷新画像验证

**已发布——v0.19（体验跃迁）**：`suggest_profile()`——没打标签的 segment
直接要一份带证据的排序候选清单（零依赖方法电池跑自身历史 + v0.18 行业
带加分 + 近三年增速超 40% 时把 `regime_shift_tech` 钉在首位的
hypergrowth 先验）；它只排序和解释，标签仍由分析师来打。demo 补全教学
闭环（没标签 → 推荐对 → 1.0% sMAPE），`docs/profile-catalog.md` 由注册表
*自动生成*——目录页永不与代码脱节。
